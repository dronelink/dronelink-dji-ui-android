...
open class DJIWidgetFactory(val context: Context, session: DroneSession?) : WidgetFactory(session) {
    ...
    override fun createCameraMenuWidget(primary: Boolean): Widget {
        return DJICameraMenuWidget()
    }

    override fun createCameraSettingsExposureWidget(primary: Boolean): Widget {
        return DJICameraSettingsExposureWidget()
    }
}